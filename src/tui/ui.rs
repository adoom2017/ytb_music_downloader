...
fn render_header(f: &mut Frame, area: Rect) {
    let title = Paragraph::new(Line::from(vec![
        ...
        Span::styled("音乐下载器", Style::default().fg(Color::Rgb(196, 181, 253)).add_modifier(Modifier::BOLD)),
...
fn render_search_bar(f: &mut Frame, app: &TuiApp, area: Rect) {
    ...
    let border_color = if focused { ACCENT } else { Color::Rgb(30, 30, 46) };
    ...
    let cursor_char = if focused && !app.is_searching { "▌" } else { "" };
...
fn render_results(f: &mut Frame, app: &TuiApp, area: Rect) {
    ...
    let (results, search_state, _query) = if let Some(s) = &state_guard {
        (s.search_results.clone(), s.search_state.clone(), s.last_query.clone())
    } else {
    ...
    let items: Vec<ListItem> = results
        ...
        .enumerate()
        .map(|(i, v)| {
            let selected_marker = if i == app.result_selected { "▶ " } else { "  " };
...
fn render_downloads(f: &mut Frame, app: &TuiApp, area: Rect) {
    ...
    let border_color = if focused { ACCENT2 } else { Color::Rgb(30, 30, 46) };
    ...
    let block = Block::default()
        ...
        .title(Span::styled(
            ...
            Style::default().fg(if focused { ACCENT2 } else { MUTED }).add_modifier(Modifier::BOLD),
        ))
        .style(Style::default().bg(BG));
    ...
    for (i, task) in tasks[start..].iter().enumerate() {
        ...
        let row_area = Rect { x: inner.x, y, width: inner.width, height: 3 };
...
fn render_download_item(f: &mut Frame, task: &crate::state::DownloadTask, area: Rect) {
    ...
    let sep_area = Rect { x: area.x, y: area.y + 2, width: area.width, height: 1 };

    let (status_label, status_color, progress) = match &task.status {
        DownloadStatus::Queued => ("队列中", MUTED, 0.0_f64),
        DownloadStatus::Downloading { progress } => ("下载中", ACCENT, (*progress as f64).min(100.0)),
...
fn render_status_bar(f: &mut Frame, app: &TuiApp, area: Rect) {
    ...
    let help = "Tab:切换面板  ↑↓:移动  d/Enter:下载  q:退出";
    let left = Paragraph::new(msg)
        .style(Style::default().fg(Color::Rgb(148, 163, 184)).bg(Color::Rgb(15, 15, 22)));
...
fn render_settings_popup(f: &mut Frame, app: &TuiApp) {
    ...
    let area = Rect::new(x, y, popup_width.min(size.width), popup_height.min(size.height));
    ...
    let current_level = app.shared.try_lock().map(|s| s.current_log_level.clone()).unwrap_or_else(|_| "info".into());

    let lines = vec![
        Line::from("日志级别 (按数字键切换):"),
        Line::from(vec![
            Span::styled(" [1] trace  ", if current_level == "trace" { Style::default().fg(SUCCESS) } else { Style::default().fg(MUTED) }),
            Span::styled(" [2] debug  ", if current_level == "debug" { Style::default().fg(SUCCESS) } else { Style::default().fg(MUTED) }),
        ]),
        Line::from(vec![
            Span::styled(" [3] info   ", if current_level == "info" { Style::default().fg(SUCCESS) } else { Style::default().fg(MUTED) }),
            Span::styled(" [4] warn   ", if current_level == "warn" { Style::default().fg(SUCCESS) } else { Style::default().fg(MUTED) }),
        ]),
        Line::from(vec![
            Span::styled(" [5] error  ", if current_level == "error" { Style::default().fg(SUCCESS) } else { Style::default().fg(MUTED) }),
        ]),
        Line::from(""),
        Line::from(vec![Span::raw(" 日志目录: "), Span::styled(app.config.log_dir.to_string_lossy().to_string(), Style::default().fg(ACCENT2))]),
        Line::from(vec![Span::raw(" 保留天数: "), Span::styled(format!("{} 天", app.config.log_keep_days), Style::default().fg(ACCENT2))]),
        Line::from(""),
        Line::from(vec![Span::styled("          [s/Esc] 关闭          ", Style::default().fg(MUTED))]),
    ];
